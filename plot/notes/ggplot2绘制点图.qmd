...
filter(gapminder::gapminder, year == "2007") %>%
  select(-year) %>%
        ggplot() +
            geom_point(aes(x = gdpPercap, y = lifeExp, size = pop, fill = continent, color = continent), stroke = 1.5, shape = 21, alpha = 0.4) +
            labs(x = "Gdp per Capita", y = "Life Expectancy", title = "Bubble Plot") +
