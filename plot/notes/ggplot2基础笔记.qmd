...
以年为单位的日历图可以通过`ggTimeSeries`包中的`stat_calendar_heatmap()`函数结合ggplot2图层（注意：映射mapping只支持 date 和 fill 变量）绘制。其中横坐标为月份（每一个板块为一个月），纵坐标表示天数。对于横跨多年的日度数据，我们可以通过分面展示。绘图的关键是使用`as.integer(strftime())`日期型处理组合函数获取某天对应所在的年份、月份、周数等数据信息（或者使用lubridate包处理）。
...
# ggsave("plot/notes/output_img/calendar_chart.png", plot = last_plot(), dpi = 90)
```
### 时间序列螺旋图

![](output_img/calendar_chart.png)
